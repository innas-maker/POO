...
public class EclairagePage extends JPanel {

    private InspectionFrame frame;

    public EclairagePage(InspectionFrame frame) {
        this.frame = frame;
    ...
    private JPanel createLeftPanel() {
        ...
        JLabel num = new JLabel("03");
        num.setFont(new Font("SansSerif", Font.BOLD, 48));
        num.setForeground(Color.WHITE);

        JLabel title = new JLabel("ECLAIRAGE");
    ...
    private JScrollPane createCenterPanel() {
        ...
        CatalogueParametres.creerParametres().stream()
                .filter(p -> p.getCategorie() == CategorieControle.ECLAIRAGE)
                .forEach(p -> {
    ...
    private JPanel createBottomPanel() {
        ...
        JButton suivant = new JButton("Suivant → Freinage");
        suivant.setBackground(Color.WHITE);
        suivant.setForeground(Color.BLACK);
        suivant.setFocusPainted(false);

        suivant.addActionListener(e -> frame.afficherPage("Freinage"));
