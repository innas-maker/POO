...
public class DocumentsPage extends JPanel {

    private InspectionFrame frame;

    public DocumentsPage(InspectionFrame frame) {
        this.frame = frame;
    ...
    private JPanel createLeftPanel() {
        ...
        JLabel num = new JLabel("01");
        num.setFont(new Font("SansSerif", Font.BOLD, 48));
        num.setForeground(Color.WHITE);

        JLabel title = new JLabel("DOCUMENTS ADMINISTRATIFS");
    ...
    .filter(p -> p.getCategorie() == CategorieControle.DOCUMENTS_ADMINISTRATIFS)
    .forEach(p -> {
    ...
    private JPanel createParametrePanel(ParametreControle parametre) {
        ...
        contenu.setBackground(new Color(20, 20, 20));
    ...
    box.addActionListener(e -> {
        if (box.isSelected()) {
            System.out.println(
                "Défaut : " + defaut.getDescription() +
                " | Niveau : " + defaut.getNiveau()
            );
        }
    });
    ...
    private JPanel createBottomPanel() {
        ...
        JButton suivant = new JButton("Suivant → Moteur");
        suivant.setBackground(Color.WHITE);
        suivant.setForeground(Color.BLACK);

        suivant.addActionListener(e -> frame.afficherPage("Moteur"));
